...
| <a name="input_cloudwatch_logs_group_names_to_ship"></a> [cloudwatch\_logs\_group\_names\_to\_ship](#input\_cloudwatch\_logs\_group\_names\_to\_ship) | List of CloudWatch Log Group names that you want to ship to Splunk. | `list(string)` | `null` | no |
...
| <a name="input_name_cloudwatch_logs_to_ship"></a> [name\_cloudwatch\_logs\_to\_ship](#input\_name\_cloudwatch\_logs\_to\_ship) | Name of the CloudWatch Log Group that you want to ship to Splunk (single log group; leave empty to not create the subscription filter; see var.cloudwatch\_logs\_group\_names\_to\_ship for creating subscription filters for multiple log groups). | `string` | `null` | no |
| <a name="input_nodejs_runtime"></a> [nodejs\_runtime](#input\_nodejs\_runtime) | Runtime version of nodejs for Lambda function | `string` | `"nodejs18.x"` | no |
