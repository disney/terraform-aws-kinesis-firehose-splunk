...
| <a name="input_name_cloudwatch_logs_to_ship"></a> [name\_cloudwatch\_logs\_to\_ship](#input\_name\_cloudwatch\_logs\_to\_ship) | name of the CloudWatch Log Group that you want to ship to Splunk. | `string` | n/a | yes |
...
| <a name="input_lambda_processing_buffer_size_in_mb"></a> [lambda\_processing\_buffer\_size\_in\_mb](#input\_lambda\_processing\_buffer\_size\_in\_mb) | Lambda processing buffer size in mb. It was noticed that the Lamba appeared to set this by default to `0.256` upon creation. | `number` | `null` | no |
